...
if (($handle = fopen("prix - Sheet1.csv", "r")) !== FALSE) {
    $header = fgetcsv($handle, 1000, ",");
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        $leMac[] = [$data[0], $data[1], $data[2], $data[3]];
        $leTeteBrulee[] = [$data[6], $data[7], $data[8], $data[9]];
    }
    fclose($handle);
}
?>
...
<body>
    ...
    <main>
        <section id="données">
            <div class="données-mac">
                ...
                <table border="1">
                    <tr>
                        ...
                        <th>Prix au kg</th>
                        <th>Kg nécessaire</th>
                        <th>Coût pour une fournée</th>
                    </tr>
                    <?php foreach ($leMac as $row): ?>
                    <tr>
            ...
            <div class="données-tete-brulee">
                <h2>Le Tête Brûlée</h2>
                <table border="1">
                    <tr>
                        <th>Composant</th>
                        <th>Prix au kg</th>
                    ...
                    <?php foreach ($leTeteBrulee as $row): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($row[0]); ?></td>
                        <td><?php echo htmlspecialchars($row[1]); ?></td>
                        <td><?php echo htmlspecialchars($row[2]); ?></td>
