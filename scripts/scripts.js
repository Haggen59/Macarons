document.addEventListener("DOMContentLoaded", () => {
    ...
    const calculateFinalPrice = () => {
        ...
        const livraison = parseFloat(livraisonElement.innerText.replace('Livraison: ', '').replace('€', ''));
        let promoPercent = JSON.parse(localStorage.getItem('promoValidee')) || 0;
        const promoCode = document.getElementById('promo-code').value;
    
        if (promoCode === 'Bienvenue10') {
            promoPercent = -0.1;
            localStorage.setItem('promoValidee', promoPercent);
        } else {
            promoPercent = 0;
            localStorage.setItem('promoValidee', promoPercent);
        }
